#TODO: record history and provide in search window
import gi
...
class SearchDialog(Gtk.Dialog):
    ...
    def __init__(self, msg=None, engine_key=None):
        ...
        if self.msg is not None:
            ...
            msg.set_markup(
                '<span foreground="red">' + '<big>' + self.msg + '</big></span>')
            box.add(msg)

        prompt = Gtk.Label(label="")
        prompt.set_markup(
            "<big>Enter title to search for...</big>")  # Pango markup
...
class ListingWin(Gtk.Window):
    ...
    def __init__(self, torrent_list, engine_key):
        ...
        self.grid.attach(self.scrollable_treeList, 0, 0,
                         8, 10)  #  row,col,(h,w cells)
        self.grid.attach_next_to(
            self.buttons[0],
            self.scrollable_treeList, Gtk.PositionType.BOTTOM, 1, 1)
