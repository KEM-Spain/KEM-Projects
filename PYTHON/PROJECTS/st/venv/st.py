...
osd_username = 'miller_kurt_e@yahoo.com'
osd_password = 'OnePlusOne=2'
osd_language = 'en'
...
# Search and download a subtitle even if a subtitle file already exists.
...
def checkSubtitlesExists(path):
    ...
    if opt_language_suffix in ('on', 'auto'):
        for ol in opt_languages:
            for l in list(ol.split(',')):
    ...
    for ext in extList:
        for lang in lngList:
            subPath = path.rsplit('.', 1)[0] + lang + '.' + ext
